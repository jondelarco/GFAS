#!/bin/ksh
#-----------------------------------------------------------------------
# Nombre : xitfj_smv_mdm.sh
#
# Fecha: 09/12/2020     Autor: Integracion 
#
#-----------------------------------------------------------------------
#
# Script que recoge fichero de SMV, lo carga en la tabla intermedia en PINTERF
# lo filtra y transforma en otro fichero para el MDM de clientes (EroskiClub)
#
##-----------------------------------------------------------------------
...
# Variables locales
DIR_DATOS=$DATOS/in/eroskiclub/smv
FICH_ENT="eroski_movimientos_tarjetas_*.csv"
LOGSQLLOADER=$DATOS/log/SMVtoMDM_`date '+%Y%m%d%H%M%S'`.log
SH_NAME="xitfj_smv_mdm"
DIR_SALIDA="/home/itf/datos/out/erkclub"
FECHA_FICH_OUT=`date +%Y%m%d%H%M%S`
FICH_SALIDA=${DIR_SALIDA}/SMVtoMDM_SALDOS_CANCELADOS.${FECHA_FICH_OUT}.DAT # Si no especificamos extension, pone .lst por defecto
SQL_NAME="SMV_MDM"
...
CONTROL=$FUENTE/sql/SMV_MDM.ctl
...
then
#*************VACIADO DE ITFCOLADM.T_SMV_MDM ANTES DE LA CARGA ***********************************************
sqlplus ${ORAUSER}/${ORAPASS}@${ORAHOST}  <<eof >>${LOG_EJEC}
truncate table ITFCOLADM.T_SMV_MDM;
commit;
eof
...
if [ ${RESULTADO2} -ne 0 ]
then
   /usr/bin/SUTSPILO APR_ITF :${SH_NAME}.sh:Aborted. Error de carga de datos en tabla T_SMV_MDM
   mensaje="Error de carga de datos en tabla T_SMV_MDM"
...
#   fi
   ...
   echo >>${LOG_EJEC}
   echo "Carga de datos en tabla T_SMV_MDM ejecutado correctamente" >> ${LOG_EJEC}
   echo >>${LOG_EJEC}
...
#   fi
   ...
   then
      ...
      then
       ...
       /usr/bin/SUTSPILO APR_ITF :${SH_NAME}.sh:Aborted. Error de extraccion de datos en tabla T_SMV_MDM
       mensaje="Error de extraccion de datos en tabla T_SMV_MDM"
