CREATE OR REPLACE PACKAGE PK_ITF_163_1061
AS
--
-- INTERFAZ DE MENSGAL.
   ----------------------
-- Nombre del Paquete: PK_ITF_163_1061.
-- Descripcion: Interfaz para el envio de la mensajeria de localizaciones a los SGAs
--
-- CONTROL DE MODIFICACIONES.
   --------------------------
-- Fecha             Nombre              Descripcion del cambio.
------------------------------------------------------------------
-- 30/01/2009    Mikelats Garcia         Creacion del paquete
------------------------------------------------------------------
   ...
   TYPE t_destinos IS TABLE OF men_loc_sga_destinos%ROWTYPE
      INDEX BY BINARY_INTEGER;

   --
--  Nombre del procedimiento: P_OBTENER_163_1061.
-- Descripcion: Procedimiento que extrae los datos de localizaciones para la mensajeria pendiente de ser enviada.
--
-- CONTROL DE MODIFICACIONES
-- Fecha         Nombre               Descripcion del cambio
-------------------------------------------------------------
-- 30/01/2009    Mikelats Garcia      Creaci¿n del procedimiento
   PROCEDURE p_obtener_163_1061 (
      p_cod_plataforma    IN       itf_gestion_men_clientes.cod_plataforma%TYPE,
      p_cod_cab_mensaje   IN       cab_mensaje.cod_cab_mensaje%TYPE,
      p_restauracion      IN       itf_gestion_men_clientes.flg_restauracion%TYPE,
      p_msgid                IN       errores_integracion.MSGID%TYPE,
      p_men_loc_sga       OUT      v_t_t_men_loc_sga,
      p_cod_error         OUT      pk_arq_errores.t_cod_error,
      p_msg_error         OUT      pk_arq_errores.t_msg_error
   );

--
--  Nombre del procedimiento: P_ACTUALIZAR_163_1061.
-- Descripcion: Procedimiento que actualiza el flag de enviado a 'S' de la tabla men_loc_sga.
--
-- CONTROL DE MODIFICACIONES
-- Fecha         Nombre               Descripcion del cambio
-------------------------------------------------------------
-- 30/01/2009    Mikelats Garcia      Creaci¿n del procedimiento
   /*PROCEDURE p_actualizar_163_1061 (
      p_id_lanzamiento   IN       men_loc_sga.id_lanzamiento%TYPE,
      p_cod_error        OUT      pk_arq_errores.t_cod_error,
      p_msg_error        OUT      pk_arq_errores.t_msg_error
   );*/
   PROCEDURE p_obtener_men_loc_sga_destinos (
      p_cod_plataforma   IN       men_loc_sga_destinos.cod_plataforma%TYPE,
      p_destinos         OUT      t_destinos,
      p_error            OUT      pk_arq_errores.t_r_error
   );
END PK_ITF_163_1061;
/
CREATE OR REPLACE PACKAGE BODY PK_ITF_163_1061
AS
--
--  Nombre del procedimiento: P_OBTENER_163_1061.
-- Descripcion: Procedimiento que extrae los datos de localizaciones
--              para la mensajeria pendiente de ser enviada.
--
-- CONTROL DE MODIFICACIONES
-- Fecha         Nombre               Descripcion del cambio
-------------------------------------------------------------
-- 30/01/2009    Mikelats Garcia      Creaci¿n del procedimiento
   ...
   BEGIN
      ...
      p_obtener_men_loc_sga_destinos (p_cod_plataforma, v_destinos, v_error);
...
--                  || NVL (v_query_destinos_n3, ' ');
		 ...
		 -- DTWXMER-1356. Se envuelve todo el bloque de destinos entre parentesis
		 -- para que el AND con la fecha de envio aplique al conjunto completo
		 v_query_destinos :=
			 k_parentesis_abrir || v_query_destinos || k_parentesis;
...
-- 30/01/2009    Mikelats Garcia      Creaci¿n del procedimiento
   ...
   BEGIN
      ...
      UPDATE men_loc_sga
         SET flg_enviado = 'E'
       WHERE id_lanzamiento = p_id_lanzamiento;

      COMMIT;
   EXCEPTION
      WHEN OTHERS
      THEN
         p_cod_error := pk_apr_constantes.k_error_oracle;
         p_msg_error :=
                      'Error actualizando flag enviado ( ' || SQLERRM || ' )';
   END;*/
   PROCEDURE p_obtener_men_loc_sga_destinos (
      p_cod_plataforma   IN       men_loc_sga_destinos.cod_plataforma%TYPE,
      p_destinos         OUT      t_destinos,
   ...
   BEGIN
      ...
      p_error.v_cod_error := pk_apr_constantes.k_no_error;
      p_error.v_msg_error := 'Correcto';

      --Se obtienen los destinos de la plataforma
      SELECT *
      BULK COLLECT INTO p_destinos
        FROM men_loc_sga_destinos
       WHERE cod_plataforma = p_cod_plataforma;

      IF p_destinos.COUNT = k_cero
      THEN
         RAISE NO_DATA_FOUND;
      END IF;
   EXCEPTION
      ...
      THEN
         p_error.v_cod_error :=
                               pk_apr_constantes.k_error_datos_no_encontrados;
         p_error.v_msg_error :=
               'No hay destinos especificos para la plataforma '
            || p_cod_plataforma;
      WHEN OTHERS
      THEN
         p_error.v_cod_error := pk_apr_constantes.k_error_oracle;
         p_error.v_msg_error := 'Error obteniendo destinos';
   END;
END PK_ITF_163_1061;
/
