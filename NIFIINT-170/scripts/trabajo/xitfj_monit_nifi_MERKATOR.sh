...
# Modificado por: Almudena Rodriguez
# Cambios efectuados:
# - Añadido parametro 6 opcional para envío de correos adicionales.
#	En la tabla pram_mail de PINTERF se han añadido los correos de las personas a las que tenga que llegar el mail
#	Cod_int=6011 (MONITO_NIFI), Cod_ORI=424 (NIFI), COD_DEST=XXX
# - Añadido envío de correos con alertas en caso de acumulación de mensajes, sean críticos o no.
#-----------------------------------------------------------------------
#Codigo de Retorno :
...
# En Linux las variables de BBDD se recuperan tras los par�metros
# Cargamos las variables de conexion a la base de datos
# Se conecta a PINTERF con ITFCOLADM
...
then  
	...
	if [ "${CRITICO}" != "N" ]; then
		ASUNTO="ALERTA CRITICA - Circuito de nifi con mensajes acumulados."
	else
		ASUNTO="ALERTA - Circuito de nifi con mensajes acumulados."
	fi

	# Si el parametro 6 viene informado se evian los correos a los destinatarios recuperados en la consulta p_select_mails
	if [ ! -z "$cod_dest" ]
	then
		# Recuperamos los correos a los que se enviara el mail
		p_select_mails		
		echo -e "${CUERPO}" | mail -s "${ASUNTO}" -c "$CORREOS_ADICIONALES" "pses-integracion@eroski.es"
	else
		# Correo base
	...
	#si es critico se genera incidencia en PLATON
	if [ "${CRITICO}" != "N" ]
	then
			finlog 1
			exit 1
